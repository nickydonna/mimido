...
import { alarmFromString } from '$lib/parser';
import { json } from '@sveltejs/kit';
import { subMinutes, differenceInDays, parseISO, startOfDay, addDays, setMinutes, setHours, getMinutes, getHours } from 'date-fns/fp';
...
export async function POST({ request }) {
  ...
  const run = openai.beta.threads.runs.stream(thread.id, { assistant_id: 'asst_wWo8F5NQrkyUqP4SXow79tgF' })
  return new Promise((resolve) => {
    run.on('messageDone', (message) => {
      const content = /** @type {import('openai').OpenAI.Beta.Threads.Messages.TextContentBlock}*/ (message.content[0]);
